...
std::array<double, 9> make_random_triangle(std::mt19937_64& rng)
{
...
std::array<double, 3> make_intersecting_ls(const std::array<double, 9>& coords, std::mt19937_64& rng)
{
...
static void BM_CutTriangle(benchmark::State& state)
{
    std::mt19937_64 rng(20260307);

    const auto coords = make_random_triangle(rng);
    ...
    for (auto _ : state)
    {
        ...
        {
            cutcells::cell::triangle::cut<double>(coords, 3, ls, "phi=0", out, false);
