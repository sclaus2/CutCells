...
def mesh_view_from_pyvista(grid: pv.UnstructuredGrid, tdim: int) -> cutcells.MeshView:
    """Wrap a pyvista UnstructuredGrid as a cutcells MeshView.

    Parameters
    ----------
    grid : pv.UnstructuredGrid
        Source grid.  Points must already by stored as (N, 3) float64.
    tdim : int
        Topological dimension of the cells (e.g. 2 for triangles/quads).

    Returns
    -------
    cutcells.MeshView (MeshView_float64 alias)
        Zero-copy view backed by the pyvista arrays; the pyvista grid must
        remain alive as long as the MeshView is used.
    """
    coordinates = np.asarray(grid.points, dtype=np.float64)  # (N, 3)
    connectivity = np.asarray(grid.cell_connectivity, dtype=np.int32)  # flat CSR
    offsets = np.asarray(grid.offset, dtype=np.int32)  # (ncells+1,)
    cell_types_np = np.asarray(grid.celltypes, dtype=np.int32)  # VTK type IDs
...
def create_triangle_mesh(x0: float, y0: float, x1: float, y1: float, N: int):
    """Create a 2-D Delaunay triangle mesh on [x0,x1]×[y0,y1].

    Returns a pyvista UnstructuredGrid with triangular cells.
    """
...
def main():
    ...
    grid = create_triangle_mesh(-1.0, -1.0, 1.0, 1.0, N)
    print(f"  nodes: {grid.n_points},  cells: {grid.n_cells}")

    mesh_view = mesh_view_from_pyvista(grid, tdim=2)
    print(
